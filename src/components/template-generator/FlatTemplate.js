        ...
        return (
            <div className="flex justify-center">
                <div className="m-4 p-4 border-2 border-dotted w-1/2">
                    ...
                    {SHEETNAMES.map((sheetname, index) => <InputMetadataRowCount
                        key={index}
                        name={sheetname}
                        handleChange={this.handleChange}/>)}
                    <p className="ml-8 text-grey-dark text-xs italic">File sheet can be downloaded
                        in next tab, with already registered file information</p>
                    <div className="bg-teal-lightest border-2 border-blue m-4 p-4 w-1/3">
                        {(Object.values(this.state).length > 0)
                            ? <ExcelDownloadFlat
                                    data={formatMetadataOptionsForFlatLayout(metadataOptions, this.state)}/>
...
const InputMetadataRowCount = (props) => (<Input
    className="m-4"
    icon='tags'
    iconPosition='left'
    label={{
