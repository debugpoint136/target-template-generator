...
import { createNeo4jManifestQuery } from './api';
// import { makeAllWorkSheets, restructureSheetFillouts, fillRows } from './utils';
// import app from "../../fire";

const Excel = require('exceljs/dist/es5/exceljs.browser');

class MergeManifestDownload extends Component {
    state = {
    ...
    handleClick = (e, {name}) => {
        fire.database().ref(`merge_requests/${this.state.uid}/${name}`).once('value', downloadedObj => {
            ...
            if (Object.keys(downloadedObj).length === 0) {
                ...
                notify.show('👀 at this point, fetched merge request will be used to stitch completed manifest', 'warning');
                // createNeo4jManifestQuery(FIREBASE_DATA, FIREBASE_USER, FIREBASE_LAB);
...
export default MergeManifestDownload;